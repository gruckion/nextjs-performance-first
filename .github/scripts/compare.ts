...
const filesize = (bytes: number, options?: any) => {
...
const renderBudgetPercentage = (
  ...
  const budgetChangeText = ` _(${renderStatusIndicator(
    budgetChange,
    budgetPercentIncreaseRed
  )}${
...
const markdownTable = (
  ...
  const headers = [
    ...
    showBudget && `% of Budget (\`${filesize(options.budget!)}\`)`,
  ]
    .filter(Boolean)
    .join(" | ");
  ...
  const rows = items.map((d) => {
    ...
    const previousBudgetPercentage =
      globalBundleBase && d.gzipDiff && options.budget
        ? Number(
            (
              ((globalBundleCurrent!.gzip + d.gzip - d.gzipDiff) /
                options.budget) *
              100
            ).toFixed(2)
          )
